...
def sampleNPoints(leftDown, rightUp, n):
  randAr = np.random.rand(2, n)

  randArX = randAr[0] * (rightUp[0] - leftDown[0])
  randArY = randAr[1] * (rightUp[1] - leftDown[1])

  midX = (rightUp[0] + leftDown[0]) / 2
  midY = (rightUp[1] + leftDown[1]) / 2

  randArX += leftDown[0] - midX
  randArY += leftDown[1] - midY
...
def ratio(n, radius):
  sampled = sampleNPoints((x1, y1), (x2, y2), n)
  x = 0
  for i in range(n):
    if (sq(sampled[0][i]) + sq(sampled[1][i])) <= radius:
      x += 1
