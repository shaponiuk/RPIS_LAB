...
def checkFormula(variables, formula):
  for f in formula:
    if f > 0:
      if variables[f - 1] <= 0:
        return False
...
def findNotUsedVariables(p, varCount):
  return varCount - len(p)

def nOfSuccessful(n, varCount, formula):
  l = [ randB(varCount) for _ in range(n) ]
  lAr = np.array(l)
  sum = 0
  i = 0
  ...
  for p in formula:
    ...
    for x in range(initCount):
      if not checkFormula(l[x], p):
        indexesToStay.append(x)
      else:
        indexesToDelete.append(x)

    goodAr = lAr[indexesToDelete]
    #lAr = lAr[indexesToStay]

    sN = n // 10
    oVar = findNotUsedVariables(p, varCount)

    if i > 0:
      count = 0
      for _ in range(sN):
        randF = np.random.randint(i)
        randA = np.random.randint(len(goodAr))

        if not checkFormula(goodAr[randA], formula[randF]):
          count += 1
